    ...
    stage ('Push Image') {
      withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'aws-creds', usernameVariable: 'AWS_ACCESS_KEY_ID', passwordVariable: 'AWS_SECRET_ACCESS_KEY']]){
        withEnv(["AWS_REGION=${AWS_REGION}"]){
          try {
            sh "source /data/jenkins/awscli/bin/activate; if [ `aws ecr describe-repositories | grep repositoryName | grep \"${app_name}\" | wc -l` -eq 0 ]; then aws ecr create-repository --repository-name ${app_name}; fi"
            ...
            sh 'source /data/jenkins/awscli/bin/activate; export AWS_DEFAULT_REGION=${AWS_REGION}; eval `aws ecr get-login | cut -d" " -f1,2,3,4,5,6,9`'
